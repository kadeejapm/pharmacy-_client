...
function AdminEditBlog4() {
    ...
    const navigate = useNavigate();
      ...
      const editBlog = async(e) =>{
        e.preventDefault()
        try {
            const response = await axios.put(`http://localhost:3000/api/admin/blog4/${id}`,{image:image,date:date,name:name,description:description},
            {headers:{
    ...
    <div className="">
        <form action="" onSubmit={editBlog}>
            ...
            <div className="flex flex-col w-[200px] gap-3 m-auto mt-3">
                ...
                <input type="submit" value="submit"  className='text-green-600 border-green-600 border text-xs sm:text-base hover:bg-green-600 hover:text-white'/>
            </div>
        </form>
    </div>
    </>
